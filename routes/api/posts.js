...
// @desc    Unlike a post
// @access  Private
router.route('/unlike/:postId').put(auth, async (req, res) => {
  try {
    const post = await Post.findById(req.params.postId);
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!post) {
        return res
          .status(404)
          .json({ msg: `No post found with the id ${req.params.postId}` });
      }
...
// @desc    Delete comment
// @access  Private
router.route('/comment/:postId/:commentId').delete(auth, async (req, res) => {
  try {
    const post = await Post.findById(req.params.postId);
    if (!post) {
